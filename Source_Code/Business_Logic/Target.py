...
class Target:
    ...
    def __init__(self, ip: str = "127.0.0.1", ports_range: str = "0-65535"):
        assert self.__class__.check_ip(ip) == True, "Invalid IPv4 Address, check_ip(ip) failed"
        assert self.__class__.check_ports(ports_range) == True, "Invalid Ports Range, check_ports(ports_range) failed"
        self.ip = ip
